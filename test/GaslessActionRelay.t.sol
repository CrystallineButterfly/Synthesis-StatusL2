...
import {GaslessActionRelay} from "src/GaslessActionRelay.sol";

contract GaslessActionRelayTest is Test {
    GaslessActionRelay private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_statusl2");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new GaslessActionRelay(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        GaslessActionRelay freshController = new GaslessActionRelay(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);

...
MODULE = importlib.import_module('agents.status_field_runner')
...
class ProjectContextTest(unittest.TestCase):
    ...
    def test_primary_names_match(self) -> None:
        spec = MODULE.build_project_spec()
        self.assertEqual(spec.primary_python_module, 'status_field_runner')
        self.assertEqual(spec.primary_contract_name, 'GaslessActionRelay')
